...
FOLDERS_TO_TRANSLATE = [
    "curriculum/i18n-curriculum/curriculum/challenges/swahili/blocks/learn-quadratic-equations"
]

# Tunable concurrency (overridable via environment variables).
# Optimized for NVIDIA RTX 3060 Ti 8GB:
#   - FILE_WORKERS: 7 parallel files (balances GPU utilization without OOM)
#   - CHUNK_WORKERS: 9 parallel chunks per file (keeps GPU fed with work)
FILE_WORKERS = int(os.environ.get("TRANSLATE_FILE_WORKERS", "7"))
CHUNK_WORKERS = int(os.environ.get("TRANSLATE_CHUNK_WORKERS", "9"))

# Model is fully configuration-driven (NEVER hardcoded at a call site).
# Default changed to qwen3.5:9b-q5_K_M for optimal speed/quality on RTX 3060 Ti.
# Switch freely via the TRANSLATE_MODEL env var or the --model CLI flag.
DEFAULT_MODEL = os.environ.get("TRANSLATE_MODEL", "qwen3.5:9b-q5_K_M")
...
# OPTIMIZATION: Persistent HTTP session for connection pooling.
# Reduces connection overhead for concurrent requests to Ollama.
# Using a session allows HTTP keep-alive and connection reuse across threads.
import urllib3
http_pool = urllib3.PoolManager(
    maxsize=CHUNK_WORKERS + FILE_WORKERS,  # Pool size for concurrent requests
...
def _call_llm(prompt, model):
    ...
    otherwise calls the local Ollama HTTP API with connection pooling.
    Returns the raw response string (stripped) or raises on transport error.
    
    OPTIMIZED: Uses persistent urllib3 connection pool instead of creating
    a new connection per request. This significantly reduces latency for
    concurrent requests.
    """
    ...
    data = {
        ...
        "options": {
            "temperature": 0.3,
            # OPTIMIZATION: Increased context window from 4096 to 8192.
            # RTX 3060 Ti 8GB can handle larger context efficiently with qwen3.5:9b-q5_K_M.
            # Larger context reduces chunking overhead for longer paragraphs.
            "num_ctx": 8192
        }
    }

    # OPTIMIZED: Use persistent connection pool instead of urllib.request
